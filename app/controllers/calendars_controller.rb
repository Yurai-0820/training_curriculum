class CalendarsController < ApplicationController
  ...
  def get_week
    ...
    7.times do |x|
      today_plans = []
      plans.each do |plan|
        today_plans.push(plan.plan) if plan.date == @todays_date + x-1
      end
      ...
      days = { :month => (@todays_date + x-1).month, :date => @todays_date.day + x-1, :plans => today_plans, :wday => wdays[wday_num + x-1]}
